...
```{r chunk4 reading pre-processed expression data, echo=FALSE, include=FALSE}
# read preprocessed expression matrics in R 
preProc_matx <- list.files("LINE1-BLCA/preProc_tcag_matrices/", pattern = "RDS")
...
for (mtx in preProc_matx) {
  ...
  assign(sub("\\.RDS$", "", mtx), readRDS(paste0("LINE1-BLCA/preProc_tcag_matrices/", mtx)))
}
```
...
# running GSVA
data.gsva <- gsva(data.vst, master_geneList, mx.diff=TRUE, verbose=FALSE,min.sz=5,max.sz=500) #$es.obs
# extract intrested signature: Immune + Epithelial + Epigenetics and ...
epithelial_index <- grep("AG.Robertson.2017.BLCA", rownames(data.gsva))
immport_index <-  grep("Immport", rownames(data.gsva))
combes_index <- grep("Combes", rownames(data.gsva))
epigen_idx <- c(36:55)
...
epithelial_sig <- data.frame(t(data.gsva[epithelial_index, ]))
epithelial_sig <- epithelial_sig[rownames(sig_TEs),]
## Immport sig
immport_sig <- data.frame(t(data.gsva[immport_index, ]))
immport_sig <- immport_sig[rownames(sig_TEs),]
## Immune_pheno from combes
combes_sig <- data.frame(t(data.gsva[combes_index, ]))
combes_sig <- combes_sig[rownames(sig_TEs),]
## Epigenetics
epiGenetics_sig <- data.frame(t(data.gsva[epigen_idx, ]))
epiGenetics_sig <- epiGenetics_sig[rownames(sig_TEs),]

#
#all(rownames(sig_TEs) == rownames(epithelial_sig))
#TRUE
# Create an empty dataframe to store results
result_df <- data.frame(
  col_names_df1 = character(),
  col_names_df2 = character(),
...
for (i in 1:ncol(sig_TEs)) {
  for (j in 1:ncol(epithelial_sig)) {
    # Extract the columns
    x <- sig_TEs[, i]
    y <- epithelial_sig[, j]
    ...
    result_df <- rbind(result_df, data.frame(
      col_names_df1 = colnames(sig_TEs)[i],
      col_names_df2 = colnames(epithelial_sig)[j],
...
# select TEs
TEs_cor_epithelial <- result_df$col_names_df1[result_df$correlation_coefficient >= 0.4 &
                                                result_df$FDR < 0.05]
