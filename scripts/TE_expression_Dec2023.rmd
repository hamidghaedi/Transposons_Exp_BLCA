...
# aggregate TEs at class level
tmp <- data.frame(log2CPM[rownames(log2CPM) %in% surv_tes_fdr_0.05,])
# 
rmk_tmp <- unique(te_rmk[, c(3:5)][te_rmk[, c(3:5)]$repName %in% rownames(tmp), ])

tmp$repName <- rownames(tmp)
# joing 
tmp <- left_join(tmp, rmk_tmp)
#
sig_TEs_fam <- tmp[, -c(434, 435)] %>% 
  group_by(repFamily) %>%
  summarise_all(sum) %>%
  data.frame() 

#
rownames(sig_TEs_fam) <- sig_TEs_fam$repFamily
sig_TEs_fam <- sig_TEs_fam[, -1]
colnames(sig_TEs_fam) = gsub('[.]', "-", colnames(sig_TEs_fam))
sig_TEs_fam <- t(sig_TEs_fam)
...
for (i in 1:ncol(sig_TEs)) {
  for (j in 1:ncol(data.gsva[, c(1:80)])) {
    # Extract the columns
    x <- sig_TEs[, i]
    y <- data.gsva[, j]
