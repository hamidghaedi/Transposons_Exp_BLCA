...
## TE signature development uisng GSVA approach
...
```
## TE signature development uisng LASSO regression model
...
survData <- data.frame(
    ...
    event = ifelse(clinical_data$paper_Vital.status == "Alive", 0,
...
```
## TE signature development uisng PCA  (defining a meta gene per sample)

```{r chunk12  TE signature development using PCA , echo=FALSE}
## APPROACH 2 : USING LASSO regression model to develop the signature
library(BhGLM)

# lets define group of TEs based on their HR
surv_summary$HRgroup <- as.character(cut(surv_summary$Hazard_Ratio,
                            breaks = c(0.40,  0.70, 1.20, 3),
                            labels = c("UnFav","NotSig", "Fav")))


survGroup <- surv_summary[surv_summary$var_name %in% overlap_cor_sig_allTEs_surv_tes_fdr_0.05,]
# create list in needed format
survGroupList <- survGroup %>%
  group_split(HRgroup, keep = FALSE) %>%
  setNames(unique(survGroup$HRgroup)) %>%
  lapply(function(df) df$var_name)
# survdata
survData <- data.frame(
    barcode = clinical_data$barcode,
    event = ifelse(clinical_data$paper_Vital.status == "Alive", 0,
...
tmp <- t(log2CPM[rownames(log2CPM) %in% overlap_cor_sig_allTEs_surv_tes_fdr_0.05,])
# set order
tmp <- data.frame(tmp[survData$barcode,])
# confirm order
all(rownames(tmp)==survData$barcode)

#
#  Fit Bayesian Spike-and-Slab Lasso Model:
y <- Surv(survData$time, survData$event)
# set group
group <-survGroupList


# Fit Bayesian Cox Proportional Hazards Model
f2 <- bmlasso(x = tmp, y = y, family = "cox", ss = c(0.04, 0.5), group = group)
#

```
## Validation of TE sigs:
The validation can be done through two different approaches: (1) Examining the correlation between TE sig and interesting phenotypes (ssGSEA, CIBERSORT, ESTIMATE, xCell, Regulon score and etc ...) and (2) through KAPS algorithm

### (1) Examining the correlation between TE sig and interesting phenotypes
