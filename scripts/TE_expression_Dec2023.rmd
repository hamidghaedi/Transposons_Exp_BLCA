...
te_rmk <- readRDS("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/r_objects/te_rmk.RDS")

#
intergenicTE_rmk <- data.frame(data.table::fread("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/te_intergenic_rmsk.tsv"))


intergenicTE_rmk <- readRDS("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/r_objects/intergenicTE_rmk.RDS")
...
# 3. clinical data 
clinical_data <- readRDS("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/r_objects/clinical.rds")
...
correlateMatrix <- function(df1, df2, show_progress = TRUE) {
  ...
  #A function  to calculate the Pearson correlation between columns of two data frames, returning 
  # the correlation coefficient, p-value, and false discovery rate (FDR). 
...
aggregate_repName2repFamily <- function(expressionMatrix, te_rmk) {
  # A function to aggregate TE expression  at Family level by taking mean
  # check to see if tr_rmk is present in environment, if not rise a message 
  # and say "TE repeat masker annotation file is not available"
...
survAnalysisOnExpMat <- function(expressionMatrix, clinicalData, timeColumn, eventColumn) {
  # A function to calculate survival analysis on an expression matrix,
  # Assumptions: expressionMatrix is an expression matrix
  # clinicalData 
        # is a dataframe where the patients ids are in barcode column
        # There is event column filled with 0(no-evenet) and 1(event)
        # By default it can convert values "Alive" and "Dead" to , 0 and 1
        # A time column 
  # library
...
```{r chunk7  TE signature development survival analysis at repName level, echo=FALSE}
...
```{r chunk8  TE signature development survival analysis at repFamily level, echo=FALSE}
...
```{r chunk9  TE signature development  gene sig cor with TEs, echo=FALSE}
...
data.gsva <- readRDS("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/r_objects/data.gsva_3k_pathways.RDS")
# transpose dataframe
...
estDat = read.csv("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/estimateScoreTCGA.csv")

tmeDat = read.csv("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/tmeScoreTCGA.csv")
...
# check rownames
estDattmeDat <- estDattmeDat[substr(rownames(all_TEs),1,15),]
# cjhck orders
all(rownames(estDattmeDat)==substr(rownames(all_TEs),1,15))
...
# we may consider the following TEs to develop a TE signature score per:
...
#_____________ 2 Split by cor test result
corDat <- t(log2CPM)
...
# visualization:
plot(hc, main = "Hierarchical Clustering Dendrogram", xlab = "Variables", sub = NULL, cex = 0.6)
...
TE_geneList <- list("all_60_TEs" = overlap_cor_sig_allTEs_surv_tes_fdr_0.05,
                    "TE_signature_favorableOS" = surv_summary$var_name[surv_summary$var_name %in% overlap_cor_sig_allTEs_surv_tes_fdr_0.05 & surv_summary$Hazard_Ratio < 1],
                    "TE_signature_UnfavorableOS" = surv_summary$var_name[surv_summary$var_name %in% overlap_cor_sig_allTEs_surv_tes_fdr_0.05 & surv_summary$Hazard_Ratio > 1],
...
TE_signatureScore <- t(gsva(log2CPM, TE_geneList, 
                          min.sz=1,
                          max.sz=500,
                          method= "gsva",
                            kcdf="Gaussian"))
...
cor_TE_sigScore_gsva <- correlateMatrix(TE_signatureScore, data.gsva[, c(1:77)]) #77 selected pathways
#
tmp_TE_signatureScore <- TE_signatureScore[rownames(data.gsva),]
# correlation
cor_TE_sigScore_gsva <- correlateMatrix(TE_signatureScore, data.gsva[, c(1:77)]) #77 selected pathways
...
cibersortDAt <- readRDS("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/r_objects/cibersortDAt.RDS")
#
all(rownames(TE_signatureScore) %in% rownames(cibersortDAt))
#
cibersortDAt <- cibersortDAt[rownames(TE_signatureScore),]
#
all(rownames(TE_signatureScore)==rownames(cibersortDAt))
# perform correlation analysis 
cor_TE_sigScore_cibersort <- correlateMatrix(TE_signatureScore, cibersortDAt)
# 3- ESTIMATE + TME
estDattmeDat <- estDattmeDat[!is.na(rownames(estDattmeDat)),]

estDattmeDat <- estDattmeDat[rownames(estDattmeDat) %in% rownames(TE_signatureScore),]
#
estDattmeDat <- cibersortDAt[rownames(tmp_TE_signatureScore),]
#
tmp_TE_signatureScore <- TE_signatureScore[rownames(estDattmeDat), ]

all(rownames(tmp_TE_signatureScore)==rownames(estDattmeDat))
# perform correlation analysis 
cor_TE_sigScore_estTme <- correlateMatrix(tmp_TE_signatureScore, estDattmeDat)
cor_TE_sigScore_estTme[order(abs(cor_TE_sigScore_estTme$correlation_coefficient)),]

# 3____________ x-cell
##
xcell = data.frame(data.table::fread("C:/Users/qaedi/OneDrive - Queen's University/Documents/LINE1-BLCA/xCell_TCGA_RSEM.txt"), row.names = T)
# repair sample name in header
colnames(xcell) = gsub('[.]', "-", colnames(xcell))
xcell = t(xcell)
# tmp TE sig score
tmp_TE_signatureScore <- TE_signatureScore
rownames(tmp_TE_signatureScore) <- substr(rownames(tmp_TE_signatureScore),1,15)
#
xcell <- xcell[rownames(xcell) %in% rownames(tmp_TE_signatureScore),]
tmp_TE_signatureScore <- tmp_TE_signatureScore[rownames(tmp_TE_signatureScore) %in% rownames(xcell),]
#
tmp_TE_signatureScore <- tmp_TE_signatureScore[!duplicated(rownames(tmp_TE_signatureScore)),]
tmp_TE_signatureScore <- tmp_TE_signatureScore[rownames(xcell),]
#
# correlation
cor_TE_sigScore_xcell <- correlateMatrix(tmp_TE_signatureScore, xcell)

# 4 _______ regulon score PLACE HOLDER

```

```{r chunk11  TE signature development using LASSO , echo=FALSE}
## APPROACH 2 : USING LASSO regression model to develop the signature
survData <- data.frame(
    barcode = clinicalData$barcode,
    event = ifelse(clinicalData$paper_Vital.status == "Alive", 0,
                     ifelse(clinicalData$paper_Vital.status == "Dead", 1, NA)),
    time = as.numeric(clinicalData$paper_Combined.days.to.last.followup.or.death))

tmp <- t(log2CPM[rownames(log2CPM) %in% overlap_cor_sig_allTEs_surv_tes_fdr_0.05,])
# set order
tmp <- tmp[survData$barcode,]
# confirm order
all(rownames(tmp)==survData$barcode)
#
lassoDat <- cbind(survData, tmp)
lassoDat <- lassoDat[, -1]
...
```{r chunk12  TE signature development KAPS analysis to cluster samples, echo=FALSE}
